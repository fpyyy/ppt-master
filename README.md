...
> **Locked SVG Templates** — prepare five polished SVG pages (`title.svg`, `toc.svg`, `chapter.svg`, `content.svg`, `ending.svg`) with custom placeholders such as `{{PPTTitle}}`, then use `/create-template` to turn them into a reusable locked template. The tool infers the `content.svg` workspace and writes sanitized XML for theme inspection. Runtime agents read only `template_contract.json`, not the SVG source, so embedded images stay out of prompt context. See [Templates Guide →](./docs/templates-guide.md).
...
| 🎨 | [Templates Guide](./docs/templates-guide.md) | Create and use locked SVG templates with runtime contracts |

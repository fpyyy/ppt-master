...
| `bit_standard` | Scenario | 北京理工大学毕业答辩、论文开题、中期汇报、课题汇报、学院正式展示 | `#006C39` | Formal, restrained, campus-branded academic defense style |
...
6. Validate the template directory with `.\.venv\Scripts\python.exe scripts/svg_quality_checker.py templates/layouts/<template_name> --format ppt169`
7. Register the new template by running `.\.venv\Scripts\python.exe scripts/register_template.py <template_id>` — it derives the `summary` and `keywords` index entry from `design_spec.md` and refreshes the Quick Index above

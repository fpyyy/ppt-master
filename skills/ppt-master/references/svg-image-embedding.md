...
2. Image Acquisition (Step 5):
   - Pending + ai  → Image_Generator uses Codex imagegen   → Generated
...
4. Preview: .\.venv\Scripts\python.exe -m http.server -d <project_path> 8000 → /svg_output/<filename>.svg
...
```bash
.\.venv\Scripts\python.exe -m http.server -d <project_path> 8000
...
```bash
.\.venv\Scripts\python.exe scripts/svg_finalize/embed_images.py <svg_file>                         # Single file
.\.venv\Scripts\python.exe scripts/svg_finalize/embed_images.py <project_path>/svg_output/*.svg    # Batch
.\.venv\Scripts\python.exe scripts/svg_finalize/embed_images.py --dry-run <project_path>/svg_output/*.svg  # Preview
```

...
```bash
.\.venv\Scripts\python.exe ${SKILL_DIR}/scripts/source_to_md/web_to_md.py <URL>
```
...
`<project_name>` is the user's chosen project identifier (typically `<format>_<topic_slug>`, e.g. `ppt169_joe_hisaishi`); `--copy` preserves the research artifacts at their original paths after import.

...
| `${SKILL_DIR}/scripts/svg_text_fit.py` | SVG text overflow detection and no-font-size-change wrapping |
| `${SKILL_DIR}/scripts/svg_layout_checker.py` | Structural layout geometry check for generated SVGs |
| `${SKILL_DIR}/scripts/total_md_split.py` | Speaker notes splitting |
| `${SKILL_DIR}/scripts/finalize_svg.py` | SVG post-processing (unified entry) |
| `${SKILL_DIR}/scripts/svg_to_pptx.py` | Export to PPTX |
| `${SKILL_DIR}/scripts/pptx_postprocess.py` | PPTX DrawingML post-processing after export |
...
|---|---|
| An explicit path to a locked SVG template directory (e.g. `skills/ppt-master/templates/layouts/bit_locked/`, or any path that resolves to a directory containing `design_spec.md`, `template_contract.json`, and one or more page SVGs) | Copy the entire directory under `<project_path>/templates/<template_id>/`; read only `design_spec.md` and `template_contract.json`, not SVG source |
...
The path may live anywhere. Location is irrelevant; what matters is that the user named a locked SVG template directory with `design_spec.md` and `template_contract.json`.

```bash
TEMPLATE_DIR=<user-supplied path>
TEMPLATE_ID=<basename of TEMPLATE_DIR>
mkdir -p <project_path>/templates/${TEMPLATE_ID}
cp -R ${TEMPLATE_DIR}/* <project_path>/templates/${TEMPLATE_ID}/
```
...
|---|---|---|
| `ai` | `references/image-generator.md` | Codex `imagegen` skill |
...
> On acquisition failure, do NOT halt — follow the Failure Handling rule in [image-base.md](references/image-base.md) §6: retry once, then mark the row `Needs-Manual`, report to user, and continue to the checkpoint above.
...
**Pre-generation Batch Read (Mandatory)**: before the first SVG, read the locked template contract (`template_contract.json`) when `spec_lock.template.engine = locked_svg`, and batch-read every distinct chart SVG referenced in `spec_lock.page_charts` (plus any §VII backup charts). Do NOT read template page SVGs for locked templates. See executor-base.md §1.0.

**Per-page spec_lock re-read (Mandatory)**: before **each** SVG page, `read_file <project_path>/spec_lock.md` and use only its colors / fonts / icons / images, plus the per-page `page_rhythm` / `page_layouts` / `page_charts` lookups. Locked template pages are generated by `svg_template.py apply` from placeholder JSON and workspace fragments, not by reading template SVG source. See executor-base.md §2.1.

**Locked TOC / chapter title budgets (Mandatory)**: before writing placeholder
fill JSON for locked `toc` or `chapter` pages, read each `SectionTitle*`
placeholder's `text_fit` in `template_contract.json`. These titles are direct
text replacements, so they MUST be shortened to the declared weighted character
budget before `svg_template.py apply`; do not rely on wrapping or font-size
changes. Budgets marked `enforce: always` are hard template constraints.
...
**Text Fit Gate (Mandatory)** — after all SVGs, BEFORE `svg_quality_checker.py` and speaker notes:
```bash
.\.venv\Scripts\python.exe ${SKILL_DIR}/scripts/svg_text_fit.py <project_path> --fix
```
- This gate measures text bounding boxes against card rectangles, template workspaces, and the canvas.
- `--fix` may wrap simple overflowing text without changing `font-size`; Latin words must not be split into letter fragments.
- If any overflow remains, return to Visual Construction for that page: wrap manually, expand the container, shorten wording, or re-layout. Re-run this command until it exits with 0 overflow issues.
- **Forbidden**: reducing `font-size` to make text fit.

**Layout Structure Gate (Mandatory)** — after Text Fit Gate passes, BEFORE `svg_quality_checker.py` and speaker notes:
```bash
.\.venv\Scripts\python.exe ${SKILL_DIR}/scripts/svg_layout_checker.py <project_path>
```
- This gate checks structural geometry in common generated layouts, including hub-spoke center alignment, card-grid consistency, and timeline axis alignment.
- If any structural issue remains, return to Visual Construction for that page and re-layout. Do not paper over geometry errors with manual visual offsets.
- Executor-authored structural groups MUST expose a stable `id` and SHOULD include `data-layout`, `data-center`, and related layout metadata so this gate can validate the intended model.

**Quality Check Gate (Mandatory)** — after Text Fit Gate and Layout Structure Gate pass, BEFORE speaker notes:
```bash
...
- [x] svg_text_fit.py passed (0 overflow issues)
- [x] svg_layout_checker.py passed (0 structural issues)
...
**Step 7.4** — PPTX post-processing (DrawingML fixes after export):
```bash
.\.venv\Scripts\python.exe ${SKILL_DIR}/scripts/pptx_postprocess.py <project_path>
```

This step currently centers numeric-only text boxes vertically inside the PPTX
(`a:bodyPr anchor="ctr"`), which fixes circle/badge/section numbering produced
from locked templates. It works at PPTX level because SVG coordinates only
position the text box; PowerPoint's internal textbox alignment controls where
the actual glyphs sit inside that box.

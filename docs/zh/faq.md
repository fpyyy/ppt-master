...
可以。在策略师阶段选择"AI 生图"方案后，PPT Master 默认使用 Codex `imagegen` 技能生成配图，并把结果放进项目的 `images/` 目录。你也可以使用自己的图片——只需放到项目的 `images/` 目录下即可。

## Q: 没有生图 API Key，还能配图吗？

可以。AI 生图行默认使用 Codex `imagegen`，PPT Master 不再需要自定义 `IMAGE_BACKEND`。你也可以在策略师的"图片方案"步骤选择"网络图片"。PPT Master 内置了零配置的 `image_search.py`，在 Openverse 和 Wikimedia Commons 中搜索可商用的开放许可图片（无需 API Key）。零配置搜索适合作为兜底：能直接用，但图片质量不稳定，容易出现普通用户上传、构图随意、清晰度一般的素材。
...
```bash
.\.venv\Scripts\python.exe skills/ppt-master/scripts/svg_to_pptx.py <project> -t push       # 换转场效果
.\.venv\Scripts\python.exe skills/ppt-master/scripts/svg_to_pptx.py <project> -t none       # 关闭转场
.\.venv\Scripts\python.exe skills/ppt-master/scripts/svg_to_pptx.py <project> -a none       # 关闭页内动画
.\.venv\Scripts\python.exe skills/ppt-master/scripts/svg_to_pptx.py <project> --animation fade        # 改用单一效果（仍是默认级联）
.\.venv\Scripts\python.exe skills/ppt-master/scripts/svg_to_pptx.py <project> --animation-trigger on-click   # 改为单击触发，演讲者控制节奏
```
...
- **使用 Codex `imagegen` 进行 AI 生图**：配图水平是 deck 整体观感的最大变量。用占位级的网络图片和用真正贴合内容的 AI 生成图，视觉效果完全是两个量级。

如果你看到的效果差强人意，先对照以下几点检查你的配置，再下结论：用的什么模型？上下文开了多大？有没有启用图片生成？同样的工作流，Claude Opus 配 100 万 token 上下文和 Codex 生成配图的结果，和小参数开源模型配零配置的结果，是截然不同的体验。
...
想把已经设计好的 SVG 页面制作成 PPT Master 可调用的模板？按以下步骤操作：

**第一步 — 准备 SVG 文件夹**

准备正好五个 SVG 文件：`title.svg`、`toc.svg`、`chapter.svg`、`content.svg`、`ending.svg`。需要替换的文字用自定义占位符，例如 `{{PPTTitle}}`、`{{PPTSubtitle}}`、`{{PageTitle}}`；`svg_template.py create` 会自动推断 `content.svg` 的正文工作区。

**第二步 — 让 AI 创建模板**

使用 AI 编程代理（Claude Code、Codex 等），要求它使用 **PPT Master 的 `/create-template` 工作流**处理这个 SVG 文件夹。提供的信息越详细，效果越好，例如：
...
不需要一次提供所有细节——AI 代理会通过对话追问补齐缺失信息。

**第三步 — 等待完成**

AI 代理会运行 `svg_template.py inspect`、`svg_llm_xml.py` 和 `svg_template.py create`，生成 `design_spec.md`、原始 SVG 副本、`llm_xml/*.xml` 和 `template_contract.json`。后续生成 PPT 时，智能体只读取契约和设计说明，不读取模板 SVG 源码。

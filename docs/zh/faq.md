...
每个页面保存为 `.svg`。需要替换的文字用自定义占位符，例如 `{{PPTTitle}}`、`{{PPTSubtitle}}`、`{{PageTitle}}`。

正文页必须显式标出工作区：优先使用 `data-ppt-workspace="main"`；如果元素本身没有 `x/y/width/height`，补充 `data-ppt-workspace-bbox="x y width height"`。
...
AI 代理会运行 `svg_template.py inspect` 和 `svg_template.py create`，生成 `design_spec.md`、原始 SVG 副本和 `template_contract.json`。后续生成 PPT 时，智能体只读取契约和设计说明，不读取模板 SVG 源码。
